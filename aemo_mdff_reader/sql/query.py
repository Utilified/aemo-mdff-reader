...
COLUMNS = {
    IMPORT_TABLE: ["filename", "VersionHeader", "DateTime", "FromParticipant", "ToParticipant"],
    NMI_TABLE: ["NMI", "NMIConfiguration"],
    CHANNEL_TABLE: [
        "NMI",
        "RegisterID",
        "NMISuffix",
    ...
    INTERVAL_DATA_TABLE: ["valueID", "value", "intervalID"],
    ...
    INTERVAL_B2B_TABLE: ["TransCode", "RetServiceOrder", "ReadDateTime", "IndexRead", "eventID"],
...
class QueryBuilder:
    ...
    def insert_query(table, data):
        ...
        for instance in data:
            list_value = [
                (
                    QueryBuilder.COLUMN_GEN % value
                    if "@" not in value and "STR" not in value
                    else value
                )
