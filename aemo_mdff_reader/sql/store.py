    ...
    def load_reader(self, reader):
        ...
        with self.__connection.cursor() as cursor:
                ...
                if subrecord.RecordIndicator == "200":
                    ...
                    for subsubrecord in subrecord.subrecords:
                        if subsubrecord.RecordIndicator == "300":
                            interval_data = [
                                (
                                    key[0].replace("IntervalValue", ""),
                                    subsubrecord.__getattr__(key[0]),
                                    intervalID,
                                )
                                for key in subsubrecord.intervals
                            ]
                            data_interval.append(
                                [
                                    [
                                        f'STR_TO_DATE("{subsubrecord.IntervalDate}", "%Y%m%d")',
            ...
            for row in range(len(data_channel)):
                ...
                cursor.execute(
                    """SELECT typeID FROM interval_type WHERE channelID = %s AND importID = %s""",
                    (channelID, importID),
                )
                ...
                for interval_row in data_intervals[row]:
                    ...
                    cursor.execute(query)
                    cursor.execute(
                        "SELECT intervalID FROM `interval` WHERE typeID = %s AND IntervalDate = %s"
                        % (typeID, interval_row[0][0])
                    )
